...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .btn-gold { background:var(--accent); border:none; color:var(--primary); font-weight:700; padding:0.6rem 1.5rem; border-radius:6px; text-decoration:none; font-size:0.9rem; }
...
<div class="main">
    <div class="page-title">
        ...
        <a href="{{ route('cliente.mantenimientos.pagar', $venta->id) }}" class="btn-back">
    ...
    <div class="layout">
        ...
        <div class="detalles-card">
            <div class="detalles-title"><i class="bi bi-receipt"></i> Detalle del Pago</div>
            <div class="monto-grande">{{ number_format($venta->precio, 2) }} BOB</div>
            <div class="info-row">
                <span class="info-label">Servicio</span>
                <span class="info-value">{{ $venta->tipoMantenimiento->nombre ?? '—' }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">Cementerio</span>
                <span class="info-value">{{ $venta->espacio->cementerio->nombre ?? '—' }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">Solicitud</span>
                <span class="info-value">{{ $venta->fecha_solicitud->format('d/m/Y') }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">Cliente</span>
                <span class="info-value">{{ $cliente->nombre }} {{ $cliente->paterno }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">CI</span>
                <span class="info-value">{{ $cliente->ci }}</span>
            </div>
            <div style="margin-top:1.5rem; padding:0.75rem; background:rgba(201,168,76,0.08); border-radius:8px; border-left:3px solid var(--accent);">
                <div style="font-size:0.75rem; color:var(--muted); line-height:1.6;">
                    ...
                    Pago procesado de forma segura a través de <strong style="color:var(--accent);">Libélula</strong>.
                </div>
            </div>
        </div>
        ...
        {{-- QR --}}
        <div class="qr-card">
            <div class="qr-title"><i class="bi bi-qr-code me-1"></i> Código QR-Simple</div>
...
<div class="success-overlay" id="successOverlay">
    ...
    <div class="success-sub">
        Tu pago de <strong>{{ number_format($venta->precio, 2) }} BOB</strong> ha sido confirmado.<br>
        Serás redirigido en unos segundos.
    </div>
    <a href="{{ route('cliente.mantenimientos.index') }}" class="btn-gold">
        <i class="bi bi-check me-1"></i>Ver mis mantenimientos
    </a>
</div>

<script>
    const verificarUrl = "{{ route('cliente.libelula.mantenimiento.verificar', $venta->id) }}";
    let intentos = 0;
    ...
    function verificarPago() {
        if (intentos >= maxIntentos) {
            document.getElementById('estadoVerificando').innerHTML =
                '<i class="bi bi-clock me-1"></i> Tiempo agotado. <a href="' + verificarUrl + '" style="color:var(--accent);">Verificar manualmente</a>';
            return;
        }
        intentos++;
        animarBarra();
        fetch(verificarUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
            .then(r => r.json())
            .then(data => {
                if (data.pagado) {
                    ...
                    setTimeout(() => { window.location.href = "{{ route('cliente.mantenimientos.index') }}"; }, 3000);
                } else {
                    setTimeout(verificarPago, 5000);
                }
            })
            .catch(() => setTimeout(verificarPago, 5000));
    }
